...
import { loginDriverSchema, type LoginDriverFormValues } from "../../types/authSchema";
import { useSignInMutation } from "../../api/api";
import { toast } from "react-toastify";
import { ROUTES } from "../../constants/routes";

const LoginDriver: React.FC = () => {
  ...
  useEffect(() => {
    if (signinMutation.status === "success") {
      toast.success(signinMutation.data?.message || "Account logged in successfully");
      reset();
  ...
  const onSubmit: SubmitHandler<LoginDriverFormValues> = async (values) => {
    try {
      const userData = {
        phone: values.phone,
      };
  ...
  return (
    <div className="p-7 h-screen flex flex-col justify-between">
      ...
      <div>
        <Link to={ROUTES.auth.login} className="bg-[#10b461] flex items-center justify-center text-white font-semibold mb-5 rounded-lg px-4 py-2 w-full text-lg placeholder:text-base cursor-pointer">
